...
Generated: 2026-05-25 | Last updated: 2026-06-02 (Phase 2 frontend redesign underway — design system, destination page partial impl, live audit bugs, new tables 037-044, branch update)
...
Full schema in Phase 2 section above. All 25 active destinations have seasons populated. 2+ seasons per destination for multi-season spots (e.g. KUZI + KASKAZI for Diani Beach, Kenya). Query directly with `destination_id` filter.

---

**destination_areas** — Neighbourhood/area intel per destination (~175 rows)
...
**kite_hubs** — Kite scene hubs (~147 rows)
...
Key columns: destination_id FK (nullable), passport_country, visa_required, visa_free_days, visa_type, cost_usd, processing_days, application_url, e_visa_available, valid_as_of.
...
Key columns: destination_id FK, service_type (rental/repair/storage), name, price_per_day_usd, stranded_risk (LOW/MEDIUM/HIGH), is_active.
...
**destination_wifi** — WiFi and connectivity (see full schema in original ref)
**nightlife_venues** — Bars and clubs (see full schema in original ref)
**no_wind_activities** — Activities for wind-free days (see full schema in original ref)
**recovery_services** — Massage, physio, sports recovery (see full schema in original ref)
**restaurants** — Dining options (see full schema in original ref)
**sim_card_options** — Mobile data options (see full schema in original ref)
**transportation** — Transport vendors (see full schema in original ref)

---

### 1C–1I. Other Tables (unchanged from previous version)

culture_safety, medical_emergency, coworking_spaces (Phase 1 schema tables — see previous reference for full schemas)

social_intel_staging, wind_community_reports, accommodation_reviews, destination_condition_updates (Phase 1 social sourcing tables)

open_meteo_raw, google_places_raw, seabreeze_raw, rome2rio_raw, noaa_copernicus_raw, approved_social_intel (Phase 1 data source tables)

kite_spots, spot_regulations, destination_kiter_logistics (Spot & logistics tables — migrations 030-032)

research_staging, destination_media, airports (Supporting tables)
...
1. KVNav (dark, sticky)
2. Hero zone (dark): ‹ EXPLORE, destination name H1, region/country, [TODO: stats band + The Catch + Best For + Skip If]
...
## 6. DATA INGESTION STATE (as of June 2, 2026)

All Phase 1 data collection complete. No changes since May 26, 2026.

- Normalized tables: 101 rows each (25 active, 76 inactive)
- destination_wind_seasons: populated for all 25 active destinations
- destination_areas: ~175 rows
- kite_hubs: ~147 rows
- schools: 183 rows across 25 destinations
- open_meteo_raw: 300 rows
- noaa_copernicus_raw: 300 rows
- Google Places: 2,264 rows, all admin-reviewed and ingested
